...
def TransShip(df):
    trans = df[df['Sell Out Province']!='Sell Out Country/Region']
    hwpd = pd.DataFrame(pd_list)    
    df = df.merge(hwpd,on='Product')
    df = df[df['Product Brand']=='HUAWEI'].pivot_table(values = 'Qty',columns='Sell Out Date',aggfunc=sum)
    df = df.dropna()
    df = df.reset_index()
    df = df.sort_values(by=df.columns[1], ascending=True)
    return df
